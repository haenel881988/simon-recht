...
const CONFIG = {
    ...
    ARCHIVE_DIR: 'c:\\apps\\website\\simon-recht\\tools\\build_check\\logfiles\\archiv',
    BUILD_LOGS_DIR: 'c:\\apps\\website\\simon-recht\\docs\\build_logs',
    CONFIG_FILE: 'c:\\apps\\website\\simon-recht\\tools\\build_check\\scripte\\build-checker-config.json',
    ...
    REPORT_OUTPUT: 'c:\\apps\\website\\simon-recht\\docs\\build_logs\\comprehensive-seo-report.md',
    ...
    SEO_STANDARDS: {
        ...
        EEAT_REQUIREMENTS: {
            EXPERTISE_INDICATORS: ['zertifiziert', 'experte', 'erfahrung', 'spezialist', 'fachkenntnisse'],
            AUTHORITATIVENESS_SIGNALS: ['autor:', 'über den autor', 'credentials', 'qualifikation'],
            TRUSTWORTHINESS_ELEMENTS: ['kontakt', 'impressum', 'datenschutz', 'testimonials'],
            EXPERIENCE_MARKERS: ['praxiserfahrung', 'case study', 'erfolgsgeschichte', 'projekt']
        },
        
        // IT-Zielgruppen-Sprache (nach ideen_checklisten.md)
        IT_LANGUAGE_PATTERNS: {
            TECHNICAL_METAPHORS: ['backup', 'firewall', 'server', 'netzwerk', 'infrastruktur'],
            MSP_TERMINOLOGY: ['managed services', 'msp', 'it-dienstleister', 'systemhaus'],
            AUDIENCE_ALIGNMENT: ['verständlich', 'praxisnah', 'nachvollziehbar', 'anwendbar']
        },
...
class BuildChecker {
    ...
    async run() {
        try {
            console.log('🤖 PROFESSIONAL BUILD & SEO CHECKER v6.0.0 Content Analytics + KI-Self-Validation STARTET...');
            ...
            console.log('');
            console.log('⚠️  KRITISCHER HINWEIS: WORTZÄHLUNG NUR MIT pnpm build GÜLTIG!');
            console.log('⚠️  VERBOTEN: Manuelle Wortzählung für Algorithmus-Training!');
            console.log('📊 Alle Wortzählungen werden automatisch in Log-Datei protokolliert!');
            ...
            console.log('✅ PROFESSIONAL BUILD & SEO CHECKER v4.0.0 ERFOLGREICH ABGESCHLOSSEN');
            console.log('🧠 AAR Learning Report generiert');
            console.log(`📊 Session Success Rate: ${(aarReport.summary.successRate * 100).toFixed(1)}%`);
            console.log(`🎯 New Patterns Learned: ${aarReport.summary.newPatterns}`);
            
        } catch (error) {
            console.error('❌ BUILD-CHECKER FEHLER:', error.message);
            this.addIssue(RATINGS.CRITICAL, 'Build-Checker Fehler', error.message, 'Checker-System reparieren');
            
            // Track error in AAR
            this.aarCollector.trackFix('system-error', {
                type: 'system_error',
    ...
    async archiveOldLogs() {
        try {
            ...
            const currentMonth = this.startTime.toLocaleString('de-DE', { month: 'long' }).toLowerCase();
    ...
    async runBuildProcess() {
        ...
        try {
            ...
            // Build-Zeit bewerten
            if (buildTime > 300000) { // 5 Minuten
                this.addIssue(RATINGS.OPTIMIZATION, 'Build-Zeit', 
            ...
            this.addIssue(RATINGS.CRITICAL, 'Build-Prozess Fehler', 
                error.message, 
    ...
    async analyzeLatestBuildLog() {
        try {
            ...
            if (buildLogFiles.length === 0) {
                this.addIssue(RATINGS.IMPORTANT, 'Build-Log fehlt', 
            ...
            this.addIssue(RATINGS.IMPORTANT, 'Build-Log Analyse Fehler', 
                error.message, 
    ...
    analyzeBuildLogContent(content) {
        ...
        const cssWarningMatches = content.match(/\[lightningcss minify\].*?not recognized/g);
        if (cssWarningMatches) {
            cssWarningMatches.forEach(warning => {
                this.addIssue(RATINGS.OPTIMIZATION, 'CSS-Warning', 
                    warning, 
        ...
        if (tsWarningMatches) {
            this.addIssue(RATINGS.IMPORTANT, 'TypeScript Warnings', 
        ...
        if (seoScoreMatch) {
            ...
            if (score < 95) {
                this.addIssue(RATINGS.IMPORTANT, 'SEO-Score niedrig', 
                    `SEO-Score: ${score}/100`, 
                    'Organische SEO-Optimierung durchführen');
            } else {
                this.addIssue(RATINGS.INFO, 'SEO-Score excellent', 
                    `SEO-Score: ${score}/100`, 
        ...
        if (content.includes('Keyword-Dichte zu niedrig')) {
            this.addIssue(RATINGS.OPTIMIZATION, 'Keyword-Density niedrig', 
    ...
    async analyzeContentStructure() {
        try {
            const contentDir = path.join(CONFIG.PROJECT_ROOT, 'src', 'content', 'blog');
            ...
            for (const file of blogFiles) {
                if (file.endsWith('.md')) {
                    ...
                    if (wordCount < this.config.seoChecks.minimumWordCount) {
                        this.addIssue(RATINGS.OPTIMIZATION, 'Content zu kurz', 
            ...
            this.addIssue(RATINGS.IMPORTANT, 'Content-Analyse Fehler', 
                error.message, 
    ...
    async validateSEOStructure() {
        try {
            ...
            try {
                ...
                this.addIssue(RATINGS.CRITICAL, 'Robots.txt fehlt', 
                    'robots.txt nicht in public/ gefunden', 
                    'Robots.txt in public/ erstellen');
            }
            
            // Sitemap prüfen (nach Build im dist Verzeichnis)
            const sitemapDistPath = path.join(CONFIG.PROJECT_ROOT, 'dist', 'sitemap.xml');
            const sitemapIndexPath = path.join(CONFIG.PROJECT_ROOT, 'dist', 'sitemap-index-extended.xml');
            ...
            try {
                ...
                this.addIssue(RATINGS.INFO, 'Sitemap', 'sitemap.xml vorhanden in dist/', 'OK');
                sitemapFound = true;
            } catch {
            ...
            try {
                ...
                this.addIssue(RATINGS.INFO, 'Sitemap Extended', 'sitemap-index-extended.xml vorhanden in dist/', 'OK');
                sitemapFound = true;
            } catch {
            ...
            if (!sitemapFound) {
                this.addIssue(RATINGS.IMPORTANT, 'Sitemap fehlt', 
                    'Keine sitemap.xml im dist/ Verzeichnis gefunden', 
                    'Sitemap-Generierung in astro.config.mjs prüfen');
            }
            
        } catch (error) {
            this.addIssue(RATINGS.IMPORTANT, 'SEO-Struktur Fehler', 
                error.message, 
    ...
    async generateActionPlan() {
        ...
        const important = this.issues.filter(i => i.priority === RATINGS.IMPORTANT);
        const optimization = this.issues.filter(i => i.priority === RATINGS.OPTIMIZATION);
        ...
        this.actionPlan = {
            ...
            organic_seo_principles: {
                ...
                build_time_note: "⏱️ Build-Zeit spielt KEINE Rolle - Qualität über Geschwindigkeit!"
            },
...
- 🔴 **KRITISCH:** ${this.actionPlan.summary.critical} (sofortige Behebung erforderlich)
- 🟡 **WICHTIG:** ${this.actionPlan.summary.important} (nächster Build-Cycle)  
- 🟢 **OPTIMIERUNG:** ${this.actionPlan.summary.optimization} (kontinuierliche Verbesserung)
...
${this.analyzedIssues.map((analyzedIssue, index) => {
...
**📋 SPECIFIC ACTIONS:**
${prompt.specificInstructions?.specific?.map(action => `- ${action}`).join('\n') || 'Standard procedure'}

**🎯 CONFIDENCE LEVEL:** ${prompt.specificInstructions?.confidence}

**✅ VALIDATION REQUIREMENTS:**
**Mandatory:**
${prompt.validationRequirements?.mandatory?.map(req => `- ${req}`).join('\n') || 'Standard validation'}

**SEO-Specific:**
${prompt.validationRequirements?.seoSpecific?.map(req => `- ${req}`).join('\n') || 'N/A'}
...
**❌ VERBOTENE Ansätze:**
${this.actionPlan.organic_seo_principles.forbidden_approaches.map(approach => `- ${approach}`).join('\n')}

**✅ ERLAUBTE Ansätze:**
${this.actionPlan.organic_seo_principles.allowed_approaches.map(approach => `- ${approach}`).join('\n')}
...
**Status:** ✅ Empfohlen
**Schritte:**
${this.actionPlan.workflow_intelligence.git_workflow.steps.map(step => `\`${step}\``).join('\n')}

**Sicherheit:** ${this.actionPlan.workflow_intelligence.git_workflow.safety_note}
` : `**Status:** ⏭️ Übersprungen - ${this.actionPlan.workflow_intelligence.git_workflow.reason}`}

### 🎯 Empfohlene Workflow-Schritte
${this.actionPlan.workflow_intelligence.recommended_steps.map((step, index) => `
**Schritt ${step.step}:** ${step.action}
- **Details:** ${Array.isArray(step.details) ? step.details.join(', ') : step.details}
...
${this.actionPlan.workflow_intelligence.ai_prompts.map(prompt => `
...
### 📊 IMPACT-SCHÄTZUNGEN
**Frontmatter-Fixes:**
- Probleme: ${this.actionPlan.workflow_intelligence.impact_estimations.frontmatter_fix.issues_count}
- Verbesserung: ${this.actionPlan.workflow_intelligence.impact_estimations.frontmatter_fix.estimated_improvement}
- Impact: ${this.actionPlan.workflow_intelligence.impact_estimations.frontmatter_fix.deployment_impact}

**Content-Optimierung:**
- Probleme: ${this.actionPlan.workflow_intelligence.impact_estimations.content_optimization.issues_count}
- Verbesserung: ${this.actionPlan.workflow_intelligence.impact_estimations.content_optimization.estimated_improvement}
- Impact: ${this.actionPlan.workflow_intelligence.impact_estimations.content_optimization.deployment_impact}

**Gesamt-Potenzial:**
- Max. Verbesserung: ${this.actionPlan.workflow_intelligence.impact_estimations.total_potential.max_improvement}
- Zeitrahmen: ${this.actionPlan.workflow_intelligence.impact_estimations.total_potential.timeframe}
- Ansatz: ${this.actionPlan.workflow_intelligence.impact_estimations.total_potential.approach}
...
${this.buildData?.buildTime ? `**Build-Zeit:** ${Math.round(this.buildData.buildTime/1000)}s` : ''}
...
${this.contentStats.fileWordDetails ? 
    this.contentStats.fileWordDetails.map(file => 
...
- **Durchschnitt pro Datei:** ${Math.round(this.contentStats.averageWordCount || 0)} Wörter
...
${this.contentStats.fileWordDetails ? 
    this.contentStats.fileWordDetails
        .filter(file => file.wordCount < CONFIG.SEO_STANDARDS.TARGET_CONTENT_LENGTH)
        .map(file => `- ${file.fileName}: ${CONFIG.SEO_STANDARDS.TARGET_CONTENT_LENGTH - file.wordCount} Wörter bis Ziel`)
...
**Zu prüfende Daten:**
- Content Stats: ${JSON.stringify(this.validationResults.kiPrompt.data.contentStats?.totalFiles || 0)} Dateien
...
**Validierungs-Fragen:**
${this.validationResults.kiPrompt.validationRequest.map(q => `- ${q}`).join('\n')}

### 🔍 KONSISTENZ-CHECKS:
${this.validationResults.consistencyChecks?.map(check => 
...
${this.validationResults.recommendations?.map(rec => 
    typeof rec === 'object' ? `- ${rec.file || rec.type}: ${rec.currentWords || rec.critical} → ${rec.expansionNeeded || rec.assessment}` : `- ${rec}`
...
${this.issues.filter(i => i.title.includes('Browser Console')).map(issue => 
...
- **Keywords mit Klicks:** ${this.csvData.searchQueries.filter(q => q.clicks > 0).length}
- **Zero-Click Keywords:** ${this.csvData.searchQueries.filter(q => q.impressions > 5 && q.clicks === 0).length}
- **High-Potential Keywords:** ${this.csvData.searchQueries.filter(q => q.impressions > 10 && q.position > 20).length}

**TOP 5 KEYWORDS nach Impressionen:**
${this.csvData.searchQueries.sort((a, b) => b.impressions - a.impressions).slice(0, 5).map(q => 
    `- **${q.query}:** ${q.impressions} Impressionen, Position ${q.position.toFixed(1)}, ${q.clicks} Klicks`
).join('\n')}

**🎯 COMPLIANCE MARKETING POTENZIAL:**
${this.csvData.searchQueries.filter(q => q.query.toLowerCase().includes('compliance')).map(q =>
    `- **${q.query}:** Position ${q.position.toFixed(1)} (${q.impressions} Impressionen) - RIESIGES AUSBAUPOTENZIAL!`
...
- **Seiten mit Klicks:** ${this.csvData.pages.filter(p => p.clicks > 0).length}
- **Zero-Click Seiten (>10 Impressionen):** ${this.csvData.pages.filter(p => p.impressions > 10 && p.clicks === 0).length}

**TOP 3 PERFORMING SEITEN:**
${this.csvData.pages.filter(p => p.clicks > 0).sort((a, b) => b.clicks - a.clicks).slice(0, 3).map(p => 
    `- **${p.url.split('/').slice(-1)[0] || 'Homepage'}:** ${p.clicks} Klicks (${p.impressions} Impressionen, CTR: ${p.ctr}%)`
).join('\n') || 'Keine Seiten mit Klicks gefunden'}

**🚨 ZERO-CLICK SEITEN MIT POTENZIAL:**
${this.csvData.pages.filter(p => p.impressions > 10 && p.clicks === 0).slice(0, 3).map(p =>
    `- **${p.url.split('/').slice(-1)[0] || 'URL'}:** ${p.impressions} Impressionen, Position ${p.position.toFixed(1)} - META-TAG OPTIMIERUNG SOFORT!`
...
- **Märkte mit Klicks:** ${this.csvData.countries.filter(c => c.clicks > 0).length}

**TOP 5 MÄRKTE:**
${this.csvData.countries.sort((a, b) => b.impressionen - a.impressionen).slice(0, 5).map(c => 
    `- **${c.country}:** ${c.impressions} Impressionen (${c.clicks} Klicks, CTR: ${c.ctr}%)`
).join('\n')}

**🇨🇭 SCHWEIZER MARKT:**
${this.csvData.countries.find(c => c.country === 'Schweiz') ? 
    `- Impressionen: ${this.csvData.countries.find(c => c.country === 'Schweiz').impressions}, Klicks: ${this.csvData.countries.find(c => c.country === 'Schweiz').clicks}, CTR: ${this.csvData.countries.find(c => c.country === 'Schweiz').ctr}%` : 
    'Keine Schweizer Daten verfügbar'}

**🇩🇪 DEUTSCHER MARKT POTENZIAL:**
${this.csvData.countries.find(c => c.country === 'Deutschland') ? 
    `- Impressionen: ${this.csvData.countries.find(c => c.country === 'Deutschland').impressions}, Klicks: ${this.csvData.countries.find(c => c.country === 'Deutschland').clicks} - ${this.csvData.countries.find(c => c.country === 'Deutschland').clicks === 0 ? 'UNGENUTZTES RIESENPOTENZIAL!' : 'Gute Performance'}` : 
    'Keine deutschen Daten verfügbar'}
` : ''}

${this.csvData?.impressions?.length > 0 ? `
### 📈 LEGACY IMPRESSIONEN-TREND:
- **Aktuelle Impressionen:** ${this.csvData.impressions[this.csvData.impressions.length - 1]?.impressions || 0}
- **Durchschnitt (7 Tage):** ${Math.round(this.csvData.impressions.slice(-7).reduce((sum, day) => sum + day.impressions, 0) / 7)} Impressionen
- **Indexierte Seiten:** ${this.csvData.impressions[this.csvData.impressions.length - 1]?.indexed || 0}
- **Nicht indexierte Seiten:** ${this.csvData.impressions[this.csvData.impressions.length - 1]?.notIndexed || 0}
` : ''}

${this.csvData?.criticalIssues?.length > 0 ? `
### ⚠️ KRITISCHE INDEXIERUNGS-PROBLEME:
${this.csvData.criticalIssues.map(issue => 
    `- **${issue.reason}:** ${issue.pages} Seiten (${issue.source})`
).join('\n')}
` : ''}
...
${this.csvData.gitRecommendations.map(rec => `
...
`;
    ...
    calculateHealthScore() {
        ...
        const { critical, important, optimization, total } = this.actionPlan.summary;
        ...
        if (this.contentStats.averageWordCount > CONFIG.SEO_STANDARDS.MIN_CONTENT_LENGTH) score += 5;
    ...
    mapPriorityToUrgency(priority) {
        switch (priority) {
            case 'CRITICAL': return 'SOFORT';
            case 'IMPORTANT': return 'WICHTIG';
            case 'OPTIMIZATION': return 'Normal';
            case 'INFO': return 'Niedrig';
    ...
    categorizeProblemsIntelligently() {
        ...
        for (const issue of this.issues) {
            ...
            } else if (issue.title.includes('Content') || issue.title.includes('Keyword')) {
                categories.content.push(issue);
            } else if (issue.title.includes('Build') || issue.title.includes('Config')) {
     ...
     */
    generateGitWorkflow() {
        const hasImportantIssues = this.issues.some(i => 
            i.priority === RATINGS.CRITICAL || i.priority === RATINGS.IMPORTANT
        );

        if (!hasImportantIssues) {
            return {
                recommended: false,
                reason: "Keine kritischen/wichtigen Probleme - Git-Workflow nicht erforderlich"
            };
        }
    ...
    generateIntelligentAIPrompts() {
        const criticalCount = this.issues.filter(i => i.priority === RATINGS.CRITICAL).length;
        const importantCount = this.issues.filter(i => i.priority === RATINGS.IMPORTANT).length;
        ...
        if (criticalCount === 0 && importantCount === 0) {
            prompts.push({
                ...
                message: "✅ Keine kritischen Probleme gefunden! Soll ich mit organischen SEO-Optimierungen fortfahren?",
        ...
        if (importantCount > 10) {
            prompts.push({
                ...
                action: "Priorisiere Frontmatter-Ergänzungen und strukturelle SEO-Verbesserungen"
            });
        }
        ...
        prompts.push({
            ...
            forbidden: "Keine Minifizierung, Performance-Tools oder künstliche Optimierungen",
            allowed: "Content-Qualität, natürliche Keywords, strukturelle Verbesserungen"
        });
    ...
    calculateImpactEstimations() {
        const frontmatterIssues = this.issues.filter(i => 
            i.title.includes('Frontmatter') || i.title.includes('Meta')
        ).length;
        
        const contentIssues = this.issues.filter(i => 
            i.title.includes('Content') || i.title.includes('Keyword')
        ).length;

        return {
            frontmatter_fix: {
                issues_count: frontmatterIssues,
                estimated_improvement: `${Math.min(frontmatterIssues * 3, 30)} Health Score Punkte`,
                deployment_impact: frontmatterIssues > 0 ? "SEO-Compliance deutlich verbessert" : "Keine Verbesserung"
            },
            content_optimization: {
                issues_count: contentIssues,
                estimated_improvement: `${Math.min(contentIssues * 2, 20)} Health Score Punkte`,
                deployment_impact: contentIssues > 0 ? "Organisches Ranking verbessert" : "Keine Verbesserung"
            },
            total_potential: {
                max_improvement: `${Math.min((frontmatterIssues * 3) + (contentIssues * 2), 40)} Health Score Punkte`,
    ...
    generateWorkflowSteps() {
        ...
        const criticalIssues = this.issues.filter(i => i.priority === RATINGS.CRITICAL);
        ...
        try {
            ...
            for (const filePath of markdownFiles) {
                ...
                if (frontmatter.focusKeyword) {
                    this.analyzeKeywordDensity(content, frontmatter.focusKeyword, relativePath);
                }
            ...
            console.log(`✅ Content-Analyse abgeschlossen: ${markdownFiles.length} Dateien analysiert`);
            
        } catch (error) {
            console.error('❌ Content-Analyse Fehler:', error.message);
            this.addIssue(RATINGS.CRITICAL, 'Content-Analyse Fehler', error.message, 'Content-Verzeichnis prüfen');
        }
    }
    ...
    async validateUTF8Encoding() {
        ...
        try {
            ...
            if (this.utfIssues.length === 0) {
                ...
                console.log(`⚠️ UTF-8 Probleme in ${this.utfIssues.length} Dateien gefunden`);
            }
            
        } catch (error) {
            console.error('❌ UTF-8 Validierung Fehler:', error.message);
            this.addIssue(RATINGS.CRITICAL, 'UTF-8 Validierung Fehler', error.message, 'Encoding-System prüfen');
        }
    }
    ...
    async optimizeSitemap() {
        ...
        try {
            const sitemapExists = await fs.access(CONFIG.SITEMAP_PATH).then(() => true).catch(() => false);
            ...
            this.addIssue(RATINGS.IMPORTANT, 'Sitemap-Optimierung Fehler', error.message, 'Build-Prozess und Sitemap-Konfiguration prüfen');
        }
    }
    ...
    async validateRobots() {
        ...
        try {
            const robotsExists = await fs.access(CONFIG.ROBOTS_PATH).then(() => true).catch(() => false);
            ...
            if (robotsContent.includes('Disallow: /') && !robotsContent.includes('Allow:')) {
                validationIssues.push('Komplette Seiten-Blockierung erkannt (Disallow: /)');
            }
            ...
            this.addIssue(RATINGS.OPTIMIZATION, 'Robots.txt Validierung Fehler', error.message, 'Robots.txt Setup prüfen');
        }
    }
    ...
    async generateComprehensiveSEOReport() {
        ...
        try {
            const deploymentSafe = this.issues.filter(issue => 
            ...
            console.log(`📊 Zusammenfassung: ${this.issues.length} Probleme gefunden`);
            ...
            this.addIssue(RATINGS.CRITICAL, 'SEO Report Fehler', error.message, 'Report-System reparieren');
        }
    }
    ...
    countWords(text) {
        ...
        console.log('🔢 WORTZÄHLUNG: Ausschließlich via pnpm build für Algorithmus-Training!');
    ...
    validateFrontmatter(frontmatter, filePath) {
        ...
        // Titel-Länge prüfen
        if (frontmatter.title && frontmatter.title.length > CONFIG.SEO_STANDARDS.MAX_TITLE_LENGTH) {
            this.addIssue(
                RATINGS.IMPORTANT,
                'Titel zu lang',
        ...
        if (frontmatter.description && frontmatter.description.length > CONFIG.SEO_STANDARDS.MAX_DESCRIPTION_LENGTH) {
            this.addIssue(
                RATINGS.IMPORTANT,
    ...
    analyzeKeywordDensity(content, focusKeyword, filePath) {
        ...
        const wordCount = this.countWords(content);
        const keywordMatches = content.toLowerCase().match(new RegExp(focusKeyword.toLowerCase(), 'g')) || [];
        ...
        if (density < 1.0) {
            this.addIssue(
                ...
                `${filePath}: "${focusKeyword}" Dichte ${density.toFixed(1)}% (Ziel: ${CONFIG.SEO_STANDARDS.TARGET_KEYWORD_DENSITY}%)`,
            ...
            this.addIssue(
                ...
                `${filePath}: "${focusKeyword}" Dichte ${density.toFixed(1)}% (Max empfohlen: 2.5%)`,
    ...
    analyzeContentPrinciples(content, filePath) {
        ...
        const aidaKeywords = {
            attention: ['aufmerksamkeit', 'überraschend', 'neu', 'exklusiv', 'geheimnis'],
        ...
        for (const [phase, keywords] of Object.entries(aidaKeywords)) {
            const found = keywords.some(keyword => lowercaseContent.includes(keyword));
            if (found) aidaScore++;
        }
    ...
    updateContentStats(wordCount, frontmatter) {
        ...
        if (!this.contentStats.fileWordDetails) this.contentStats.fileWordDetails = [];
        ...
        const fileName = frontmatter && frontmatter.title ? 
            frontmatter.title.toLowerCase().replace(/[^a-z0-9]/g, '-') : 
            'unknown-file';
            
        // EXAKTE WORTZÄHLUNG für Log-Datei protokollieren
        console.log(`📊 WORTZÄHLUNG PROTOKOLLIERT: ${fileName} = ${wordCount} Wörter (für Algorithmus-Training)`);
        ...
        this.contentStats.fileWordDetails.push({
            ...
            expansionPriority: this.calculateExpansionPriority(wordCount, frontmatter),
            timestamp: new Date().toISOString()
        });
    ...
    generateContentExpansionRecommendations() {
        ...
        this.contentStats.expansionRecommendations = lowWordCountFiles.map(file => ({
            fileName: file.fileName,
            title: file.title,
            currentWords: file.wordCount,
            targetWords: CONFIG.SEO_STANDARDS.TARGET_CONTENT_LENGTH,
            wordsNeeded: CONFIG.SEO_STANDARDS.TARGET_CONTENT_LENGTH - file.wordCount,
...
**Zu erweitern:** ${CONFIG.SEO_STANDARDS.TARGET_CONTENT_LENGTH - fileDetails.currentWords} Wörter
...
**Autonomous-Action:** KI darf eigenständig erweitern basierend auf User-Delegation-Preferences.`;
    ...
    generateDetailedSEOReport(data) {
        ...
        const criticalIssues = issues.filter(i => i.priority === RATINGS.CRITICAL);
        const importantIssues = issues.filter(i => i.priority === RATINGS.IMPORTANT);
        const optimizationIssues = issues.filter(i => i.priority === RATINGS.OPTIMIZATION);
...
${contentStats.expansionRecommendations && contentStats.expansionRecommendations.length > 0 ? 
  contentStats.expansionRecommendations.map((rec, index) => `
**${index + 1}. ${rec.title}** (Priorität: ${rec.priority}/10)
- **Aktuell:** ${rec.currentWords} Wörter → **Ziel:** ${rec.targetWords} Wörter (${rec.wordsNeeded} Wörter benötigt)
- **Fokus-Keyword:** ${rec.focusKeyword || 'Nicht definiert'}
- **Vorgeschlagene Erweiterungen:** ${rec.suggestedSections.slice(0, 3).join(', ')}
...
${contentStats.fileWordDetails && contentStats.fileWordDetails.length > 0 ? 
  contentStats.fileWordDetails.map(file => `
- **${file.title}:** ${file.wordCount} Wörter | Kategorie: ${file.category || 'Unbekannt'} | Priorität: ${file.expansionPriority}/10 ${file.needsExpansion ? '⚠️ Expansion empfohlen' : '✅ Länge OK'}`).join('\n') : 
...
${utfIssues.length === 0 ? 
  ...
  `❌ **FEHLGESCHLAGEN** - Encoding-Probleme in ${utfIssues.length} Dateien:\n${utfIssues.map(issue => `- ${issue.file}: ${issue.corruptions.join(', ')}`).join('\n')}`}
...
- **Keyword-Dichte:** Ziel ${CONFIG.SEO_STANDARDS.TARGET_KEYWORD_DENSITY}% für Fokus-Keywords
- **Content-Länge:** Minimum ${CONFIG.SEO_STANDARDS.MIN_CONTENT_LENGTH} Wörter für SEO-Ranking

## Technische Validierung

- **Sitemap:** Verfügbar und validiert
- **Robots.txt:** SEO-kompatibel konfiguriert
- **Content-Standards:** ${criticalIssues.length === 0 && importantIssues.length === 0 ? 'BESTANDEN' : 'FEHLGESCHLAGEN'}
- **Encoding-Integrität:** ${utfIssues.length === 0 ? 'BESTANDEN' : 'FEHLGESCHLAGEN'}
...
`;
    ...
    generateDependencyAnalysis(issue) {
        ...
        if (issueType.includes('frontmatter') || issueType.includes('meta') || issueType.includes('date') || issueType.includes('image') || issueType.includes('tags')) {
        ...
        if (issueType.includes('content') || issueType.includes('keyword') || issueType.includes('wörter')) {
        ...
        if (issueType.includes('build') || issueType.includes('css') || issueType.includes('typescript') || issueType.includes('warning')) {
        ...
        if (issueType.includes('seo') || issueType.includes('robots') || issueType.includes('sitemap')) {
    ...
    validateMissingDateField(issue) {
        const filePath = this.extractFilePathFromMessage(issue.message || issue.description);
    ...
    validateMissingImageField(issue) {
        ...
        validation += `# ODER:\n`;
        validation += `image: "/assets/blog/${filePath.split('/').pop().replace('.md', '')}.webp"\n`;
        validation += `\`\`\`\n\n`;
    ...
    async processAARLearning() {
        try {
            console.log('🧠 Processing AAR Learning Data mit Enhanced KI-Prompt Generation...');
            ...
            for (const issue of this.issues) {
                try {
                    ...
                    console.log(`🎯 Issue "${issue.type || 'Unknown'}" - Confidence: ${(confidence * 100).toFixed(1)}% - KI-Prompt generiert`);
                } catch (issueError) {
                    console.log(`⚠️ Skipping AAR analysis for issue: ${issueError.message}`);
                }
            }
            ...
            console.log(`🧠 AAR Analysis Complete: ${this.analyzedIssues.length} issues analyzed mit KI-Prompts`);
        } catch (error) {
        ...
        if (issue.filePath) {
            try {
                ...
                if (filePath && filePath.endsWith('.md')) {
                    context.frontmatterFields = this.extractFrontmatterFields(fileContent);
                }
    ...
    categorizeIssue(issue) {
        ...
        if (description.includes('frontmatter') || description.includes('meta')) return 'content_meta';
        if (description.includes('image') || description.includes('alt')) return 'media';
        if (description.includes('seo') || description.includes('keyword')) return 'seo';
        if (description.includes('link') || description.includes('url')) return 'navigation';
        if (description.includes('robots') || description.includes('sitemap')) return 'technical_seo';
    ...
    assessIssueSeverity(issue) {
        ...
        // High severity
        if (description.includes('critical') || description.includes('error')) return 'high';
        if (type.includes('missing') && description.includes('required')) return 'high';
        
        // Medium severity
        if (description.includes('warning') || description.includes('should')) return 'medium';
        if (description.includes('content length') || description.includes('keyword density')) return 'medium';
     ...
     */
    async loadCSVData() {
        console.log('📊 Lade CSV-Daten (Google Search Console + SEO Measurements)...');
        ...
        try {
            ...
            console.log(`   📈 Legacy kritische Probleme: ${this.csvData.criticalIssues.length}`);
            ...
            console.log(`   🔧 Git-Empfehlungen: ${this.csvData.gitRecommendations.length}`);
        ...
        for (const csvPath of legacyPaths) {
            try {
                if (csvPath.includes('Diagramm.csv')) {
                    ...
                    console.log(`📈 Legacy Impressionen geladen von: ${path.basename(csvPath)}`);
                } else if (csvPath.includes('Kritische Probleme.csv')) {
                    const data = await fs.readFile(csvPath, 'utf8');
                    this.parseCriticalIssuesCSV(data);
                    console.log(`⚠️ Legacy kritische Probleme geladen von: ${path.basename(csvPath)}`);
                }
            } catch (error) {
    ...
    async loadSEOMeasurementData(seoDir) {
        ...
        try {
            ...
            try {
                ...
                console.log(`🔍 Suchanfragen geladen: ${this.csvData.searchQueries.length} Keywords`);
            ...
            try {
                ...
                console.log(`📄 Seiten-Performance geladen: ${this.csvData.pages.length} URLs`);
            ...
            try {
                ...
                console.log(`🌍 Länder-Daten geladen: ${this.csvData.countries.length} Märkte`);
    ...
    parseDiagramCSV(data) {
        ...
        for (const line of lines) {
            if (line.trim()) {
                const [datum, nichtIndexiert, indexiert, impressionen] = line.split(',');
        ...
        const avgImpressions = recent.reduce((sum, day) => sum + day.impressions, 0) / recent.length;
        
        if (avgImpressions < 10) {
            this.addIssue(RATINGS.CRITICAL, 'Impressionen drastisch gesunken', 
                `Durchschnittliche Impressionen: ${avgImpressions.toFixed(1)} (letzte 7 Tage)`, 
            ...
            if (line.trim()) {
                ...
                if (parts.length >= 4) {
                    ...
                    if (seitenCount > 0) {
                        ...
                        this.addIssue(RATINGS.CRITICAL, `Indexierung Problem: ${grund}`, 
    ...
    analyzeKeywordPerformance() {
        ...
        console.log(`🔍 Analysiere ${this.csvData.searchQueries.length} Keywords...`);
        
        // High Potential Keywords (hohe Impressionen, niedrige Position)
        const highPotential = this.csvData.searchQueries.filter(q => 
            q.impressions > 10 && q.position > 20 && q.clicks === 0
        );
        
        if (highPotential.length > 0) {
            this.addIssue(RATINGS.OPTIMIZATION, 'High-Potential Keywords', 
                `${highPotential.length} Keywords mit hohem Potenzial: ${highPotential.slice(0,3).map(k => k.query).join(', ')}`, 
                'Content für Top-Keywords organisch optimieren (siehe KEYWORD-EXPANSION-VORSCHLAEGE.md)');
        }
        
        // Brand Keyword Performance
        const brandKeywords = this.csvData.searchQueries.filter(q => 
            q.query.toLowerCase().includes('simon') || q.query.toLowerCase().includes('hänel')
        );
        
        if (brandKeywords.length > 0) {
            const avgPosition = brandKeywords.reduce((sum, k) => sum + k.position, 0) / brandKeywords.length;
            if (avgPosition > 5) {
                this.addIssue(RATINGS.IMPORTANT, 'Brand Keyword Position', 
                    `Durchschnittsposition für Brand Keywords: ${avgPosition.toFixed(1)}`, 
                    'Brand-Präsenz organisch stärken');
            } else {
                this.addIssue(RATINGS.INFO, 'Excellente Brand Performance', 
                    `Brand Keywords durchschnittlich auf Position ${avgPosition.toFixed(1)}`, 
                    'Brand-Performance halten');
            }
        }
        
        // Zero-Click Keywords (RIESIGES POTENZIAL!)
        const zeroClickKeywords = this.csvData.searchQueries.filter(q => 
            q.impressions > 5 && q.clicks === 0
        );
        
        if (zeroClickKeywords.length > 5) {
            this.addIssue(RATINGS.OPTIMIZATION, 'Zero-Click Keywords POTENZIAL', 
                `${zeroClickKeywords.length} Keywords ohne Klicks trotz ${zeroClickKeywords.reduce((sum, k) => sum + k.impressions, 0)} Impressionen!`, 
        ...
        if (complianceKeywords.length > 0) {
            const complianceTotal = complianceKeywords.reduce((sum, k) => sum + k.impressions, 0);
            this.addIssue(RATINGS.OPTIMIZATION, 'Compliance Marketing Potenzial', 
                `Compliance-Keywords: ${complianceTotal} Impressionen, Ø Position: ${(complianceKeywords.reduce((sum, k) => sum + k.position, 0) / complianceKeywords.length).toFixed(1)}`, 
    ...
    analyzePagePerformance() {
        ...
        const lowCTRPages = this.csvData.pages.filter(p => 
            p.impressions > 10 && p.ctr < 2 && p.clicks === 0
        );
        
        if (lowCTRPages.length > 0) {
            this.addIssue(RATINGS.OPTIMIZATION, 'Niedrige CTR Seiten', 
                `${lowCTRPages.length} Seiten mit niedrigen Klickraten trotz ${lowCTRPages.reduce((sum, p) => sum + p.impressions, 0)} Impressionen`, 
        ...
        const nonWwwPages = this.csvData.pages.filter(p => !p.url.includes('www.') && p.url.includes('simon-haenel.com'));
        
        if (wwwPages.length > 0 && nonWwwPages.length > 0) {
            this.addIssue(RATINGS.IMPORTANT, 'URL-Duplikate erkannt', 
        ...
        if (topPages.length > 0) {
            this.addIssue(RATINGS.INFO, 'Top-Performance Seiten', 
                `Beste Seiten: ${topPages.map(p => `${p.url.split('/').pop()} (${p.clicks} Klicks)`).join(', ')}`, 
    ...
    analyzeGeographicPerformance() {
        ...
        if (topMarkets.length > 0) {
            this.addIssue(RATINGS.INFO, 'Top Märkte identifiziert', 
                `Hauptmärkte: ${topMarkets.map(m => `${m.country} (${m.impressions} Impressionen)`).join(', ')}`, 
                'Geo-spezifische Content-Optimierung für Top-Märkte prüfen');
        }
        
        // Schweizer Markt Performance (Heimmarkt!)
        const swissData = this.csvData.countries.find(c => c.country === 'Schweiz');
        if (swissData && swissData.ctr > 5) {
            this.addIssue(RATINGS.INFO, 'Schweizer Markt Excellence', 
        ...
        const germanData = this.csvData.countries.find(c => c.country === 'Deutschland');
        if (germanData && germanData.impressions > 50 && germanData.clicks === 0) {
            this.addIssue(RATINGS.OPTIMIZATION, 'Deutscher Markt UNGENUTZTES Potenzial', 
    ...
    generateGitIntelligenceRecommendations() {
        ...
        const criticalIssues = this.issues.filter(i => i.priority === RATINGS.CRITICAL).length;
        ...
        if (csvIssues > 0) {
            this.csvData.gitRecommendations.push({
                ...
                urgency: 'MITTEL',
                command: 'git add docs/015_build_logs/seo_messungs_dateien/ && git commit -m "Add: SEO measurement data analysis with keyword insights"',
        ...
        this.csvData.gitRecommendations.push({
            ...
            urgency: 'MITTEL', 
            command: 'git add docs/015_build_logs/seo_messungs_dateien/KEYWORD-EXPANSION-VORSCHLAEGE.md && git commit -m "Add: Data-driven keyword expansion recommendations"',
        ...
        this.csvData.gitRecommendations.push({
            ...
            urgency: 'HOCH',
            command: 'pnpm build && git add . && git commit -m "Post-fix validation: Resolved SEO issues based on CSV analysis"',
        ...
        console.log(`🔧 ${this.csvData.gitRecommendations.length} Git-Empfehlungen generiert`);
    }
    ...
    async performLinkIntegrityAnalysis() {
        ...
        try {
            ...
            const notFoundIssues = this.csvData.criticalIssues.filter(issue => 
                issue.reason.includes('404') || issue.reason.includes('Nicht gefunden')
            );
            
            for (const issue of notFoundIssues) {
                this.addIssue(RATINGS.CRITICAL, '404-Fehler erkannt', 
            ...
            this.addIssue(RATINGS.IMPORTANT, 'Link-Analyse Fehler', 
                error.message, 
        ...
        for (const resource of problematicResources) {
            if (resource.includes('.webp')) {
                ...
                try {
                    ...
                    this.addIssue(RATINGS.INFO, '404-Ressource gefunden', 
                        `${resource} existiert in ${imagesDir}`, 
                        'Pfad-Referenzen in HTML/Markdown korrigieren');
                } catch {
                    this.addIssue(RATINGS.CRITICAL, '404-Ressource fehlt', 
    ...
    async analyzeIndexingIssues() {
        ...
        const indexingIssues = [
            ...
            { reason: 'Seite mit Weiterleitung', pages: 11, severity: RATINGS.IMPORTANT },
            { reason: 'Alternative Seite mit richtigem kanonischen Tag', pages: 25, severity: RATINGS.OPTIMIZATION },
            { reason: 'Durch "noindex"-Tag ausgeschlossen', pages: 17, severity: RATINGS.IMPORTANT },
            { reason: 'Gecrawlt – zurzeit nicht indexiert', pages: 24, severity: RATINGS.OPTIMIZATION },
            { reason: 'Gefunden – zurzeit nicht indexiert', pages: 1, severity: RATINGS.OPTIMIZATION }
        ];
        
        for (const issue of indexingIssues) {
            if (issue.pages > 0) {
                this.addIssue(issue.severity, `Indexierung: ${issue.reason}`, 
        ...
        const totalNonIndexed = indexingIssues.reduce((sum, issue) => sum + issue.pages, 0);
        if (totalNonIndexed > 50) {
            this.addIssue(RATINGS.CRITICAL, 'Massive Indexierungs-Probleme', 
    ...
    getIndexingRecommendation(reason) {
        const recommendations = {
            'Nicht gefunden (404)': 'Tote Links entfernen, Redirects einrichten, Sitemap bereinigen',
            'Seite mit Weiterleitung': 'Redirect-Ketten auflösen, direkte Links verwenden',
            'Alternative Seite mit richtigem kanonischen Tag': 'Kanonische URLs konsolidieren',
            'Durch "noindex"-Tag ausgeschlossen': 'noindex-Tags überprüfen und entfernen falls SEO gewünscht',
            'Gecrawlt – zurzeit nicht indexiert': 'Content-Qualität verbessern, interne Verlinkung stärken',
            'Gefunden – zurzeit nicht indexiert': 'Geduld - Google indexiert bald, Content-Wert erhöhen'
        };
        
        return recommendations[reason] || 'Individuelle Analyse der Indexierungs-Probleme erforderlich';
    }
    ...
    async validateInternalLinks() {
        ...
        try {
            ...
            if (totalInternalLinks > 0) {
                this.addIssue(RATINGS.INFO, 'Interne Verlinkung', 
                    `${totalInternalLinks} interne Links gefunden`, 
                    'Interne Verlinkung für SEO weiter ausbauen');
            } else {
                this.addIssue(RATINGS.OPTIMIZATION, 'Interne Verlinkung schwach', 
        ...
        try {
            ...
            for (const filePath of markdownFiles) {
                ...
                if (internalLinks.length < CONFIG.SEO_STANDARDS.MIN_INTERNAL_LINKS) {
                    this.addIssue(RATINGS.OPTIMIZATION, 'Zu wenig interne Links', 
                ...
                if (externalLinks.length > CONFIG.SEO_STANDARDS.MAX_EXTERNAL_LINKS) {
                    this.addIssue(RATINGS.OPTIMIZATION, 'Zu viele externe Links', 
    ...
    async performCSSConsistencyAnalysis() {
        ...
        try {
            ...
            this.addIssue(RATINGS.IMPORTANT, 'CSS-Analyse Fehler', error.message, 'CSS-Analyse-System reparieren');
        }
    }
    ...
    async checkBrowserConsoleErrors() {
        ...
        for (const errorData of knownErrors) {
            this.addIssue(errorData.severity, `Browser Console: ${errorData.type}`, 
                `${errorData.error}${errorData.savings ? ` (Einsparung: ${errorData.savings})` : ''}${errorData.resource ? ` - Ressource: ${errorData.resource}` : ''}`, 
                this.getBrowserErrorRecommendation(errorData.type));
        }
        
        // Organische Performance-Hinweise (ohne Tools!)
        this.addIssue(RATINGS.INFO, 'Organische Performance-Analyse', 
    ...
    async analyzeEATContent() {
        ...
        try {
            ...
            for (const file of contentFiles) {
                try {
                    ...
                    if (eatScore.overall < 60) {
                        this.addIssue(RATINGS.IMPORTANT, 'E-A-T Content-Qualität', 
                            `${path.basename(file)}: E-A-T Score ${eatScore.overall}% (niedrig)`, 
            ...
            console.log('⚠️ E-A-T Analyse temporär deaktiviert - Content-Files nicht gefunden');
        }
    }
    ...
    checkExpertise(content) {
        const indicators = CONFIG.SEO_STANDARDS.EEAT_REQUIREMENTS.EXPERTISE_INDICATORS;
        let score = 0;
    ...
    checkAuthoritativeness(content) {
        const signals = CONFIG.SEO_STANDARDS.EEAT_REQUIREMENTS.AUTHORITATIVENESS_SIGNALS;
        let score = 0;
        ...
        const authorityMarkers = ['linkedin', 'speaker', 'autor', 'gastbeitrag', 'interview'];
    ...
    checkTrustworthiness(content) {
        const elements = CONFIG.SEO_STANDARDS.EEAT_REQUIREMENTS.TRUSTWORTHINESS_ELEMENTS;
        let score = 0;
        ...
        // Transparenz-Signale
        const trustSignals = ['referenzen', 'bewertungen', 'testimonial', 'case study'];
    ...
    checkExperience(content) {
        ...
        const experienceTerms = ['jahren erfahrung', 'projekte', 'kunden betreut', 'implementiert'];
    ...
    checkITLanguageAlignment(content) {
        ...
        const metaphorExamples = ['backup', 'firewall', 'netzwerk', 'server', 'infrastruktur'];
...
`;
    ...
    async saveKIPrompt(fileName, prompt) {
        try {
            ...
            const promptFile = path.join(promptDir, `ki-prompt-${fileName}-${Date.now()}.md`);
    ...
    generateImprovementAreas(eatScore) {
        ...
        if (eatScore.expertise < 70) areas.push('❌ EXPERTISE: IT-Zertifizierungen und Fachkenntnisse fehlen');
        if (eatScore.authoritativeness < 70) areas.push('❌ AUTHORITY: Autor-Credentials und Publikationen unzureichend');
        if (eatScore.trustworthiness < 70) areas.push('❌ TRUST: Referenzen und Testimonials fehlen');
        if (eatScore.experience < 70) areas.push('❌ EXPERIENCE: Konkrete Projekt-Beispiele unzureichend');
        if (eatScore.itLanguage < 70) areas.push('❌ IT-SPRACHE: Zielgruppen-Sprache nicht optimal');
    ...
    generateEATImprovement(eatScore) {
        ...
        if (eatScore.expertise < 70) {
            improvements.push('Expertise: IT-Zertifizierungen, Fachkenntnisse prominenter darstellen');
        }
        if (eatScore.authoritativeness < 70) {
            improvements.push('Authority: Autor-Bio, LinkedIn-Profil, Sprecherstellen erwähnen');
        }
        if (eatScore.trustworthiness < 70) {
            improvements.push('Trust: Referenzen, Case Studies, Kundenbewertungen hinzufügen');
        }
        if (eatScore.experience < 70) {
            improvements.push('Experience: Konkrete Projekt-Beispiele und Erfahrungswerte');
        }
        if (eatScore.itLanguage < 70) {
            improvements.push('IT-Sprache: Technische Metaphern für bessere Zielgruppen-Ansprache');
        }
    ...
    getBrowserErrorRecommendation(errorType) {
        const recommendations = {
            'RESOURCE_404': 'Fehlende Ressource erstellen oder Referenzen entfernen - SEO-schädliche 404s vermeiden',
            'PERFORMANCE_LATENCY': 'Organische Performance: Server-Response optimieren, DNS-Auflösung verbessern',
            'IMAGE_OPTIMIZATION': 'Organische Bild-Optimierung: WebP-Format, korrekte Dimensionen, Lazy Loading',
            'RENDER_BLOCKING': 'CSS/JS organisch optimieren: Critical CSS inline, async/defer für Scripts'
        };
        
        return recommendations[errorType] || 'Organische Lösung für Browser-Problem entwickeln';
    }
    ...
    async analyzeCSSFile(filePath) {
        try {
            ...
            for (const issue of issues) {
                this.addIssue(RATINGS.OPTIMIZATION, 'CSS-Qualität', 
                    `${relativePath}: ${issue}`, 
                    'CSS organisch optimieren (keine Minifizierung!)');
            }
            
        } catch (error) {
            console.error(`❌ CSS-Datei Analyse Fehler (${filePath}):`, error.message);
        }
    }
    ...
    async performSelfValidation() {
        ...
        try {
            ...
            this.addIssue(RATINGS.CRITICAL, 'Selbstvalidierung Fehler', error.message, 'Validierungs-System reparieren');
        }
    }
    ...
    async validateContentStatsConsistency() {
        ...
        if (this.contentStats.fileWordDetails && this.contentStats.totalWords) {
            const calculatedTotal = this.contentStats.fileWordDetails.reduce((sum, file) => sum + file.wordCount, 0);
            ...
            if (Math.abs(calculatedTotal - reportedTotal) > 10) {
                ...
                this.addIssue(RATINGS.CRITICAL, 'Wortzahl-Inkonsistenz', 
        ...
        if (this.contentStats.averageWordCount && this.contentStats.totalWords && this.contentStats.totalFiles) {
            const calculatedAvg = Math.round(this.contentStats.totalWords / this.contentStats.totalFiles);
    ...
    async validateWordCountCalculations() {
        ...
        for (const file of this.contentStats.fileWordDetails) {
            ...
            if (file.wordCount && file.wordCount < CONFIG.SEO_STANDARDS.TARGET_CONTENT_LENGTH) {
                const wordsNeeded = CONFIG.SEO_STANDARDS.TARGET_CONTENT_LENGTH - file.wordCount;
    ...
    async validateIssueCategorization() {
        ...
        const criticalCount = this.issues.filter(i => i.priority === RATINGS.CRITICAL).length;
        const importantCount = this.issues.filter(i => i.priority === RATINGS.IMPORTANT).length;
        const optimizationCount = this.issues.filter(i => i.priority === RATINGS.OPTIMIZATION).length;
    ...
    async generateSelfValidationPrompt() {
        const prompt = {
            timestamp: new Date().toISOString(),
            instruction: '🤖 KI-SELBSTVALIDIERUNG: Bitte prüfe systematisch die folgenden Checker-Daten auf Inkonsistenzen',
            data: {
                contentStats: this.contentStats,
            ...
            expectedAction: 'Identifiziere Inkonsistenzen und schlage Verbesserungen vor'
        };
...
if (require.main === module) {
    console.log('🌟 Starting Professional Build & SEO Checker with Portfolio Blueprint Features...');
