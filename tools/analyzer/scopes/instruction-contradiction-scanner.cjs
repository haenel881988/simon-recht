...
class InstructionContradictionScanner {
  ...
  async loadInstructionFiles(projectRoot) {
    ...
    // Haupt-Instructions
    const mainInstructions = path.join(projectRoot, '.github', 'copilot-instructions.md');
  ...
  async performSelfContradictionCheck(projectRoot) {
    ...
    try {
      ...
      for (const file of coreFiles) {
        if (file.endsWith('.cjs') && file !== 'instruction-contradiction-scanner.cjs') {
  ...
  generateContradictionReport() {
    const totalContradictions = this.results.contradictions.length + this.results.ruleConflicts.length;
    ...
    return {
      summary: {
        status: totalContradictions === 0 ? 'KEINE_WIDERSPRÜCHE' : 'WIDERSPRÜCHE_GEFUNDEN',
        ...this.results.statistics
      },
  ...
  countBySeverity(severity) {
    const sameFile = this.results.contradictions.filter(c => c.severity === severity).length;
    const crossFile = this.results.ruleConflicts.filter(c => c.severity === severity).length;
  ...
  generatePriorityActions() {
    ...
    const critical = [...this.results.contradictions, ...this.results.ruleConflicts]
    ...
    critical.forEach((contradiction, index) => {
      actions.push({
        ...
        location: contradiction.file || `${contradiction.files?.file1} vs ${contradiction.files?.file2}`,
        scope: contradiction.scope,
        evidence: contradiction.evidence?.first_conflict || contradiction.evidence?.conflict_preview
      });
    });
...
if (require.main === module) {
  ...
  scanner.scanContradictions(projectRoot)
    .then(report => {
