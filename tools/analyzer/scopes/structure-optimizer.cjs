...
class StructureOptimizer {
  ...
  async optimizeStructure(projectRoot) {
    console.log('🏗️ STRUCTURE-OPTIMIZER: Starte intelligente Struktur-Analyse...');
  ...
  async inventarizeCurrentStructure(projectRoot) {
    ...
    this.results.currentStructure = await this.buildStructureTree(projectRoot, projectRoot);
  }
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (!entry.name.startsWith('.') && 
              ...
              entry.name !== 'dist') {
            structure.children[entry.name] = await this.buildStructureTree(rootPath, fullPath, depth + 1);
            ...
            for (const [type, count] of childStats.fileTypes.entries()) {
              structure.stats.fileTypes.set(type, (structure.stats.fileTypes.get(type) || 0) + count);
            }
          ...
          structure.stats.fileTypes.set(extension, (structure.stats.fileTypes.get(extension) || 0) + 1);
  ...
  async categorizeFile(file, projectRoot) {
    ...
    const targetLocation = this.determineTargetLocation(file, contentCategory, purposeAnalysis);
  ...
  isTextFile(extension) {
    const textExtensions = ['.md', '.txt', '.json', '.js', '.cjs', '.ts', '.astro', '.html', '.css', '.scss', '.yml', '.yaml', '.xml'];
  ...
  determineTargetLocation(file, category, purposeAnalysis) {
    ...
    switch (purposeAnalysis.category) {
      ...
      case 'config':
        if (fileName.includes('astro') || fileName.includes('tailwind') || fileName.includes('package')) {
      ...
      case 'tool':
        if (currentPath.includes('analyzer')) {
          return `tools/analyzer/${this.determineAnalyzerScope(file)}/${fileName}`;
        }
  ...
  scoreFileLocation(currentPath, targetPath) {
    ...
    // Chaos-Pattern = -50
    if (this.chaosPatterns.some(pattern => pattern.pattern.test(currentPath))) {
      score -= 50;
    }
  ...
  getChaosPatternTarget(file, chaosPattern) {
    switch (chaosPattern.action) {
      case 'ARCHIV_ODER_LÖSCHEN':
        ...
        return `docs/archive/chaos-cleanup-${new Date().toISOString().split('T')[0]}/${file.name}`;
  ...
  async generateOptimalStructure(projectRoot) {
    ...
    this.results.structureScore = totalFiles > 0 ? Math.round(totalScore / totalFiles) : 0;
  ...
  calculateMovementImpact(file) {
    ...
    if (file.purposeAnalysis.references.length > 0) {
      impacts.push(`${file.purposeAnalysis.references.length} Links müssen aktualisiert werden`);
    }
  ...
  async createLinkMapping(file, projectRoot) {
    ...
    for (const searchFile of allFiles) {
      if (this.isTextFile(searchFile.extension)) {
        const references = await this.findReferencesToFile(searchFile, file.path, projectRoot);
        if (references.length > 0) {
          mapping.references.push({
            referencingFile: searchFile.path,
            references: references,
            updateScript: this.generateLinkUpdateScript(searchFile.path, references, file.path, file.targetLocation)
          });
        }
      }
    }
  ...
  async findReferencesToFile(searchFile, targetFile, projectRoot) {
    ...
    try {
      const content = await fs.readFile(path.join(projectRoot, searchFile.path), 'utf-8');
      
      // Verschiedene Referenz-Pattern
      const patterns = [
        new RegExp(`\\[([^\\]]+)\\]\\(([^)]*${path.basename(targetFile).replace('.', '\\.')})\\)`, 'g'), // Markdown Links
        new RegExp(`["']([^"']*${path.basename(targetFile).replace('.', '\\.')})["']`, 'g'), // Quoted Pfade
        new RegExp(`\\b${targetFile.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'g') // Direkte Pfad-Referenzen
      ];
  ...
  async generateMigrationScript(projectRoot) {
    console.log('🤖 Generiere KI-Umsetzungs-Script für automatische Migration...');
    ...
    const script = {
      metadata: {
        ...
        totalLinkUpdates: this.results.linkMappings.reduce((sum, mapping) => sum + mapping.references.length, 0)
      },
      phases: []
    };
  ...
  async analyzeSelfStructure(projectRoot) {
    console.log('🔄 SELBST-ANALYSE: Structure-Optimizer analysiert eigene Platzierung...');
  ...
  generateStructureReport() {
    const summary = {
      ...
      highUrgencyMovements: this.results.movementPlan.filter(m => m.urgency === 'HOCH').length,
      totalLinkUpdatesNeeded: this.results.linkMappings.reduce((sum, mapping) => sum + mapping.references.length, 0),
      chaosPatterns: this.results.improvements.filter(i => i.type === 'CHAOS_PATTERN').length,
  ...
  calculateEstimatedImprovement() {
    ...
    return {
      ...
      status: improvedScore > 80 ? 'OPTIMAL' : improvedScore > 60 ? 'GUT' : 'VERBESSERBAR'
    };
  }
  ...
  generateStructureRecommendations() {
    ...
    if (this.results.movementPlan.filter(m => m.urgency === 'HOCH').length > 0) {
      recommendations.push({
        priority: 'HOCH',
        action: 'High-Priority Dateien verschieben',
        count: this.results.movementPlan.filter(m => m.urgency === 'HOCH').length,
...
if (require.main === module) {
  ...
  optimizer.optimizeStructure(projectRoot)
    .then(report => {
      ...
      console.log(`   Struktur-Score: ${report.summary.currentStructureScore}/100`);
      console.log(`   Bewegungen nötig: ${report.summary.totalMovementsNeeded}`);
      ...
      console.log(`   Projizierte Verbesserung: ${report.summary.estimatedImprovement.improvement} Punkte`);
      
      if (report.movementPlan.length > 0) {
        console.log('\n📋 TOP-BEWEGUNGEN:');
        report.movementPlan.slice(0, 5).forEach((movement, index) => {
          console.log(`   ${index + 1}. ${movement.sourceFile} → ${movement.targetFile}`);
      ...
      console.log(`   Aktionen: ${report.migrationScript.phases.reduce((sum, phase) => sum + phase.actions.length, 0)}`);
