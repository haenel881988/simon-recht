...
class RedundancyDetector {
    ...
    for (const [hash, files] of contentHashes.entries()) {
      if (files.length > 1) {
        this.results.duplicateFiles.push({
          ...
          totalWastedSpace:
            files.reduce((sum, f) => sum + f.size, 0) - files[0].size,
        });
      }
    }
  }
  ...
  isPotentialOrphan(file) {
    ...
    return potentialOrphanPatterns.some(
  ...
  calculateOrphanRisk(file, relativePath) {
    ...
    if (relativePath.includes("archiv") || relativePath.includes("backup"))
      risk += 0.3;
  ...
  generateRedundancyReport() {
    ...
    const totalWastedSpace = this.results.duplicateFiles.reduce(
      (sum, dup) => sum + dup.totalWastedSpace,
      0
    );
    ...
    this.results.statistics = {
      ...
      highRiskOrphans: this.results.orphanedFiles.filter((o) => o.risk > 0.7)
        .length,
    };
  ...
  generateRedundancyRecommendations() {
    ...
    if (this.results.statistics.duplicateGroups > 0) {
      recommendations.push({
        ...
        risk: "NIEDRIG",
        impact: `~${Math.round(
          this.results.statistics.estimatedSpaceSaving / 1024
        )}KB Speicher sparen`,
      });
    }
